...
def load_results(results_dir):
    all_files = [
        ...
        if f.endswith(".csv") and f.startswith("Sparsity_Q_penalty_sparsity")
    ]
...
def main():
    ...
    avg = df_all.groupby(['tau', 'formulation'])[["root_ub","root_lb","root_gap","end_ub", "end_lb", "end_gap", "nnz", "node_count", "time"]].mean()
    print(avg)

    # save
    output_path = os.path.join(current_dir, "averaged_results_penalty_sparsity.csv")
