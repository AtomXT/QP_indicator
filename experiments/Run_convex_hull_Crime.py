...
for dataset in data_list:
    ...
    for m in m_list:
        ...
        # try:
            ...
            # binaries
            z = model_hull.addMVar(n, vtype=GRB.BINARY, name="z")
            lam0 = model_hull.addMVar(n, vtype=GRB.BINARY, name="lam0")
            lamp = model_hull.addMVar(n, vtype=GRB.BINARY, name="lamp")
            lamm = model_hull.addMVar(n, vtype=GRB.BINARY, name="lamm")

            # continuous
            y_hull = model_hull.addMVar(m, vtype=GRB.CONTINUOUS, lb=-GRB.INFINITY, ub=GRB.INFINITY, name="y")

            t = model_hull.addMVar(n, vtype=GRB.CONTINUOUS, lb=-GRB.INFINITY, ub=GRB.INFINITY, name="t")
            t0 = model_hull.addMVar(n, vtype=GRB.CONTINUOUS, lb=-GRB.INFINITY, ub=GRB.INFINITY, name="t0")
            tp = model_hull.addMVar(n, vtype=GRB.CONTINUOUS, lb=-GRB.INFINITY, ub=GRB.INFINITY, name="tp")
            tm = model_hull.addMVar(n, vtype=GRB.CONTINUOUS, lb=-GRB.INFINITY, ub=GRB.INFINITY, name="tm")

            x = model_hull.addMVar(n, vtype=GRB.CONTINUOUS, lb=-GRB.INFINITY, ub=GRB.INFINITY, name="x")
            x0 = model_hull.addMVar(n, vtype=GRB.CONTINUOUS, lb=-GRB.INFINITY, ub=GRB.INFINITY, name="x0")
            xp = model_hull.addMVar(n, vtype=GRB.CONTINUOUS, lb=-GRB.INFINITY, ub=GRB.INFINITY, name="xp")
            xm = model_hull.addMVar(n, vtype=GRB.CONTINUOUS, lb=-GRB.INFINITY, ub=GRB.INFINITY, name="xm")
            ...
            # region selection
            model_hull.addConstrs(lam0[i] + lamp[i] + lamm[i] == 1 for i in range(n))

            # link to your original z (active iff +/- region)
            model_hull.addConstrs(z[i] == lamp[i] + lamm[i] for i in range(n))

            # define t = F y + c  (vector form is fine)
            model_hull.addConstr(t == F @ y_hull + c)

            # recomposition
            model_hull.addConstr(t == t0 + tp + tm)
            model_hull.addConstr(x == x0 + xp + xm)

            # inactive region: |t| <= tau, x=0
            model_hull.addConstrs(t0[i] >= -tau[i] * lam0[i] for i in range(n))
            model_hull.addConstrs(t0[i] <= tau[i] * lam0[i] for i in range(n))
            model_hull.addConstrs(x0[i] == 0 for i in range(n))

            # active + region: t >= tau, t <= U, x = -(1/Dii) t
            model_hull.addConstrs(tp[i] >= tau[i] * lamp[i] for i in range(n))
            model_hull.addConstrs(tp[i] <= U[i] * lamp[i] for i in range(n))
            model_hull.addConstrs(xp[i] == -(1.0 / Dii[i]) * tp[i] for i in range(n))

            # active - region: L <= t <= -tau, x = -(1/Dii) t
            model_hull.addConstrs(tm[i] >= L[i] * lamm[i] for i in range(n))
            model_hull.addConstrs(tm[i] <= -tau[i] * lamm[i] for i in range(n))
            model_hull.addConstrs(xm[i] == -(1.0 / Dii[i]) * tm[i] for i in range(n))

            # (optional but often helpful) keep the master t bounded explicitly too
            # this is redundant if your disaggregation is correct AND L/U are correct,
            # but it can help presolve.
            model_hull.addConstrs(t[i] >= L[i] for i in range(n))
            model_hull.addConstrs(t[i] <= U[i] for i in range(n))
            ...
            model_hull.setObjective(obj[0], GRB.MINIMIZE)

            # params (same style)
            model_hull.params.OutputFlag = 1
            model_hull.params.Threads = 8
            model_hull.params.TimeLimit = timelimit

            model_hull.optimize(record_root_lb)
            result_hull = [m, n, dataset, 'opt', root_bound[0], root_bound[1],
                          (root_bound[0] - root_bound[1]) / root_bound[0], model_hull.ObjVal, model_hull.ObjBound,
                          (model_hull.ObjVal - model_hull.ObjBound) / model_hull.ObjVal, model_hull.NodeCount,
                          model_hull.runtime]
            results.append(result_hull)
            print('--------------------------------------------------')
            print("Solve the optimal solution in the convex-hull formulation")
            print(f"The obj is {model_hull.objVal}.")
            print(
                f"The root upper bound is: {root_bound[0]}, lower bound is: {root_bound[1]}. The root gap is: {np.round(100 * (root_bound[0] - root_bound[1]) / root_bound[0], 4)}%. Runtime: {model_hull.runtime}.")
            print('--------------------------------------------------')
            ...
            print(results_df)
            results_df.to_csv(f"{current_dir}/../experiments_results/convex_hull_results_Crime.csv")
        # except:
        #     continue
