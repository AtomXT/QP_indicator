        ...
        for dataset in data_list:
            ...
            for pair in index_pair:
                ...
                if metric_pair > metric:
                    metric = metric_pair
                    best_pairs = pairs
                    best_decomposition['Di_'] = Di_
                    best_decomposition['Gi_'] = Gi_
                    best_decomposition['Fi_'] = Fi_
                    best_decomposition['Di_sum_diff_'] = Di_sum_diff_
                    best_decomposition['Gi_sum_diff_'] = Gi_sum_diff_
                    best_decomposition['Fi_sum_diff_'] = Fi_sum_diff_
                print("------------------------")
            Di_, Gi_, Fi_ = best_decomposition['Di_'], best_decomposition['Gi_'], best_decomposition['Fi_']
            Di_sum_diff_, Gi_sum_diff_, Fi_sum_diff_ = best_decomposition['Di_sum_diff_'], best_decomposition[
                'Gi_sum_diff_'], best_decomposition['Fi_sum_diff_']
            pairs = best_pairs
            ...
            for iii in range(len(psi_values)):
                ...
                for ii, pair in enumerate(pairs):
                    model_dul.addConstr(
                        t_dul[ii] >= alpha.T @ z_dul + beta.T @ x_dul + gamma[pair].T @ y_dul[pair] + psi_value[ii])
            ...
            result = [m, n, dataset, 'cut', root_bound[0], root_bound[1],
                      (root_bound[0] - root_bound[1]) / root_bound[0], model_dul.ObjVal, model_dul.ObjBound, (model_dul.ObjVal - model_dul.ObjBound) / model_dul.ObjVal, model_dul.NodeCount, model_dul.runtime]
            results.append(result)
            print('--------------------------------------------------')
...
print(results_df)
results_df.to_csv("../experiments_results/results.csv")