...
#SBATCH --time=02:00:00 ## how long does this need to run (remember different p>
#SBATCH --mem=12G ## how much RAM do you need per CPU, also see --mem=<XX>G for >
#SBATCH --job-name=path_Q_adv_exact3  ## When you run squeue -u NETID this is how you ca>
#SBATCH --output=quest_jobs/outlog/Q_sparsity_path_Q_adv_exact3_log ## standard out and standar>
...
python3 -m experiments.Run_path_Q_adversarial \
  --n_list "100,1000" \
  --timelimit "600" \
  --rep_list "2" \
  --tau_list "0.6,0.7,0.8,0.9" \
  --job_name "path_Q_adversarial_exact3"
