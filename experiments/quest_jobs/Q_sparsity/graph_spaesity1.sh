...
#SBATCH --time=24:00:00 ## how long does this need to run (remember different p>
#SBATCH --mem=8G ## how much RAM do you need per CPU, also see --mem=<XX>G for >
#SBATCH --job-name=graph_spaesity1  ## When you run squeue -u NETID this is how you ca>
#SBATCH --output=quest_jobs/outlog/Q_sparsity_graph_spaesity1_log ## standard out and standar>
...
python3 -m experiments.Run_sparse_to_dense_Q \
  --n_list "500" \
  --delta_list "0.01,0.02,0.05" \
  --tau_list "0.1" \
  --timelimit "3600" \
  --rep_list "0,1,2,3,4" \
  --job_name "graph_spaesity1"
