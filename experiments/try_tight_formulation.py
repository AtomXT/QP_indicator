...
model_ori.params.TimeLimit = 300
...
model_opt.addConstrs(-np.sqrt(2*lam[i]*D[i, i]) - 1000*z_opt[i]<= gp.quicksum(F[i, j]*y_opt[j] for j in range(m)) + c[i] for i in range(n))
model_opt.addConstrs(np.sqrt(2*lam[i]*D[i, i]) + 1000*z_opt[i] >= gp.quicksum(F[i, j]*y_opt[j] for j in range(m)) + c[i] for i in range(n))
model_opt.addConstrs(-np.sqrt(2*lam[i]*D[i, i]) + 1000*(1-z_minus[i]) >= gp.quicksum(F[i, j]*y_opt[j] for j in range(m)) + c[i] for i in range(n))
model_opt.addConstrs(np.sqrt(2*lam[i]*D[i, i]) - 1000*(1-z_plus[i]) <= gp.quicksum(F[i, j]*y_opt[j] for j in range(m)) + c[i] for i in range(n))
...
model_opt.params.TimeLimit = 300
...
z_opt_val = np.squeeze([zi.X for zi in z_opt])
thr = np.quantile(z_opt_val,0.8)
print(np.array([1.0 if v>thr else 0.0 for v in z_opt_val]))
