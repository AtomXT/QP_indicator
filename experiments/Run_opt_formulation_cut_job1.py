...
from src.utils import decomposition, get_data
        ...
        for n in n_list:
            try:
                ...
                for pair in index_pair:
                    Di_, Gi_, Fi_, Di_sum_diff_, Gi_sum_diff_, Fi_sum_diff_ = decomposition(D, G, F, [pair])
                    metric_pair = np.linalg.norm(Fi_[0], 1) / np.sum(np.count_nonzero(Fi_[0], axis=1) != 0)
                    if metric_pair > metric:
                        metric = metric_pair
                        best_pairs = [pair]
                print(f"The best pair is: {best_pairs}")
                print("------------------------")
                Di_, Gi_, Fi_, Di_sum_diff_, Gi_sum_diff_, Fi_sum_diff_ = decomposition(D, G, F, best_pairs)
                pairs = best_pairs.copy()
                # get dual variables
                model_dul = gp.Model()
                ...
                model_dul.params.BarHomogeneous = 1
                model_dul.params.QCPDual = 1
                model_dul.params.Threads = 8
                model_dul.optimize()
                ...
                z_plus = model_dul.addMVar(n, vtype=GRB.BINARY, name='z+')
                z_minus = model_dul.addMVar(n, vtype=GRB.BINARY, name='z-')
                y_dul = model_dul.addMVar(m, vtype=GRB.CONTINUOUS, lb=-GRB.INFINITY, ub=GRB.INFINITY, name='y')
                x_dul = model_dul.addMVar(n, vtype=GRB.CONTINUOUS, lb=-GRB.INFINITY, ub=GRB.INFINITY, name='x')
                t_dul = model_dul.addMVar(len(pairs), vtype=GRB.CONTINUOUS, lb=0, ub=GRB.INFINITY, name="t")
                ...
                model_dul.addConstrs(x_dul[i] <= BIG_M * z_dul[i] for i in range(n))
                model_dul.addConstrs(x_dul[i] >= -BIG_M * z_dul[i] for i in range(n))
                model_dul.addConstrs(z_dul[i] == z_plus[i] + z_minus[i] for i in range(n))
                model_dul.addConstrs(-np.sqrt(2 * lam[i] * D[i, i]) - 1000 * z_dul[i] <= gp.quicksum(
                    F[i, j] * y_dul[j] for j in range(m)) + c[i] for i in range(n))
                model_dul.addConstrs(np.sqrt(2 * lam[i] * D[i, i]) + 1000 * z_dul[i] >= gp.quicksum(
                    F[i, j] * y_dul[j] for j in range(m)) + c[i] for i in range(n))
                model_dul.addConstrs(-np.sqrt(2 * lam[i] * D[i, i]) + 1000 * (1 - z_minus[i]) >= gp.quicksum(
                    F[i, j] * y_dul[j] for j in range(m)) + c[i] for i in range(n))
                model_dul.addConstrs(np.sqrt(2 * lam[i] * D[i, i]) - 1000 * (1 - z_plus[i]) <= gp.quicksum(
                    F[i, j] * y_dul[j] for j in range(m)) + c[i] for i in range(n))
                ...
                result_cut = [m, n, dataset, 'opt_cut', root_bound[0], root_bound[1],
                          (root_bound[0] - root_bound[1]) / root_bound[0], model_dul.ObjVal, model_dul.ObjBound,
                          (model_dul.ObjVal - model_dul.ObjBound) / model_dul.ObjVal, model_dul.NodeCount,
                          model_dul.runtime]
                results.append(result_cut)
                print('--------------------------------------------------')
                ...
                print(results_df)
                results_df.to_csv(f"{current_dir}/../experiments_results/results_opt_formulation_cut_job1.csv")
            except:
                continue
