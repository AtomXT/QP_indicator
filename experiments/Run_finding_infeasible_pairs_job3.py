    ...
    for m in m_list:
        for n in n_list:
            try:
                ...
                find_infeasible_start = time.time()
                for i in range(n - 1):
                    for j in range(i + 1, n):
                        for s in ['+', '-']:
                            for t in ['+', '-']:
                                infeasible = pairwise_infeasible(F, c, np.diag(D), lam, i, j, s, t)
                                if infeasible:
                                    if (s, t) == ('+', '+'):
                                        model_opt.addConstr(z_plus[i]+z_plus[j] <= 1)
                                    elif (s, t) == ('+', '-'):
                                        model_opt.addConstr(z_plus[i]+z_minus[j] <= 1)
                                    elif (s, t) == ('-', '-'):
                                        model_opt.addConstr(z_minus[i]+z_minus[j] <= 1)
                                    elif (s, t) == ('-', '+'):
                                        model_opt.addConstr(z_minus[i]+z_plus[j] <= 1)
                                    else:
                                        print('Wrong sign!')
                find_infeasible_end = time.time()
                ...
                result_opt = [m, n, dataset, 'opt_feasibility', root_bound[0], root_bound[1],
                                 (root_bound[0] - root_bound[1]) / root_bound[0], model_opt.ObjVal, model_opt.ObjBound,
                                 (model_opt.ObjVal - model_opt.ObjBound) / model_opt.ObjVal, model_opt.NodeCount,
                                 model_opt.runtime, find_infeasible_end-find_infeasible_start]
                results.append(result_opt)
                print('--------------------------------------------------')
                print("Solve the optimal solution in the proposed formulation")
                print(f"The obj is {model_opt.objVal}.")
                ...
                print(results_df)
                results_df.to_csv(f"{current_dir}/../experiments_results/infeasible_pairs_results_job3.csv")
            except:
                continue
