...
from src.utils import decomposition, get_data, get_data_offline, load_instance_Q_tree
import os
import argparse
import json
...
for n in n_list:
        for rep in rep_list:
            Q, d, meta = load_instance_Q_tree(n, rep)
            # A = Q.toarray()
            ...
            for tau in tau_list:
                try:
                    ...
                    ## solve the problem in tree algorithm
                    start = time.time()
                    ...
                    result_opt = [n, tau, rep, 'tree', np.nan, np.nan, np.nan, tree_obj, np.nan, np.nan,
                    ...
                    print(results_df)
                    results_df.to_csv(f"{current_dir}/../experiments_results/Tree_Q_{job_name}.csv", index=False)
                except Exception as e:
                    print(f"Error: {e}")
                    continue
