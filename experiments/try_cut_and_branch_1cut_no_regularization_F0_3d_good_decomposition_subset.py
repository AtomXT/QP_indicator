...
# obj_expr = cp.lambda_max(cp.bmat([[D - cp.sum(Di), F - cp.sum(Fi)], [(F - cp.sum(Fi)).T, G - cp.sum(Gi)]]))
obj_expr = 0
for ii in range(s):
    obj_expr += cp.sum(cp.diag(Di[ii]))
    # obj_expr += cp.sum(cp.diag(Gi[ii]))
    # obj_expr += cp.lambda_min(cp.bmat([[Di[ii], Fi[ii]], [Fi[ii].T, Gi[ii]]]))
    # obj_expr += -1e-4*cp.norm(Fi[ii], 1)
# obj_expr -= cp.sum(cp.abs(cp.sum(Fi)-F))
...
# Gi_, Fi_, Di_ = [G], [F], [D]
Gi_sum_diff_, Di_sum_diff_, Fi_sum_diff_ = G - cp.sum(Gi_)+np.eye(m)*1e-6, D - cp.sum(Di_)+np.eye(n)*1e-6, F - cp.sum(Fi_)
...
for ii, pair in enumerate(pairs):
    ...
    subset = np.diag(Di_[ii]) >= 1e-6
    model_dul.addConstr(
        t_dul[ii] >= y_dul[pair].T @ Gi_[ii][np.ix_(pair, pair)] @ y_dul[pair] / 2 + x_dul[subset].T @ Di_[
            ii][np.ix_(subset, subset)] @ x_dul[subset] / 2 + x_dul[subset].T @ Fi_[ii][:, pair][subset,:]  @ y_dul[pair])
subset_all = np.diag(Di_sum_diff_) >= 1e-6
extra_term = y.T @ y / 2 + y_dul_bar.T @ Gi_sum_diff_ @ y_dul_bar/2 + x_dul_bar[subset_all].T @ Di_sum_diff_[np.ix_(subset_all, subset_all)] @ x_dul_bar[subset_all]/2 + x_dul_bar[subset_all].T @ Fi_sum_diff_[subset_all, :] @ y_dul_bar + c[subset_all].T @ x_dul_bar[subset_all] + d.T @ y_dul_bar + lam[subset_all].T @ z_dul_bar[subset_all]
...
for ii, pair in enumerate(pairs):
    ...
    model_dul.addConstr(
        t_dul[ii] >= y_dul[pair].T @ Gi_[ii][np.ix_(pair, pair)] @ y_dul[pair] / 2 + x_dul[subset].T @ Di_[
            ii][np.ix_(subset, subset)] @ x_dul[subset] / 2 + x_dul[subset].T @ Fi_[ii][:, pair][subset, :] @ y_dul[
            pair])
