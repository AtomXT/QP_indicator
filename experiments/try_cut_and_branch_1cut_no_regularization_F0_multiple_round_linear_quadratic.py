...
for iii in range(1):
    ...
    for ii, pair in enumerate(pairs):
        ...
        model_dul.addConstr(t_dul[ii] >= gamma[pair].T @ y_dul[pair] + x_dul.T @ (
                    Di_[ii] - Fi_[ii][:, pair] @ inv_Gi @ Fi_[ii][:, pair].T) @ x_dul / 2 + (
                                        Fi_[ii][:, pair] @ inv_Gi @ gamma[pair]).T @ x_dul - gamma[pair].T @ inv_Gi @
                            gamma[pair] / 2)
